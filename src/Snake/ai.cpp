	...
	while (!Q.empty()) {
		...
		for (int i = 0; i < 4; i++) {
			...
			if (xt < 0 || yt < 0 || BFCMap[xt][yt] != 0)
				...
				while (BFCBack[ttemp.x][ttemp.y].x != head.x ||
					BFCBack[ttemp.x][ttemp.y].y != head.y) {
				...
				if (ttemp.x != head.x)
					return ttemp.x - head.x > 0 ? RIGHT : LEFT;
				if (ttemp.y != head.y)
					return ttemp.y - head.y > 0 ? DOWN: UP;
			}
		}
	}
}
